...
public class CashbookService {
    ...
    public CreatedTransactionDto createTransaction(TransactionCreateRequest req) {
        ...
        if (req.txType() == TxType.EXPENSE && scope == ExpenseScope.COMMON) {
            linkMirror(t);
        }
    ...
    public void updateTransaction(Long id, TransactionUpdateRequest req) {
        ...
        if (t.getTxType() == TxType.EXPENSE) {
            if (prevScope == ExpenseScope.COMMON && nextScope != ExpenseScope.COMMON) {
                unlinkMirror(t);
            } else if (prevScope != ExpenseScope.COMMON && nextScope == ExpenseScope.COMMON) {
                txRepo.save(t);
                linkMirror(t);
                return;
            } else if (nextScope == ExpenseScope.COMMON && t.getLinkedTxId() != null) {
                syncMirror(t);
    ...
    private static void copyExpenseFields(CbTransaction from, CbTransaction to) {
        to.setTitle(from.getTitle());
        to.setAmount(from.getAmount());
        to.setCategory(from.getCategory() != null ? from.getCategory() : "");
        to.setCardName(from.getCardName() != null ? from.getCardName() : "");
